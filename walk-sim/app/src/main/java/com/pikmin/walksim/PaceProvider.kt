 ...
 * and reads a single-row cursor { playing:INT(0/1), stepsPerMin:REAL } derived from the live fix
...
class PaceProvider : ContentProvider() {
    ...
    override fun query(
        ...
        return MatrixCursor(arrayOf(COL_PLAYING, COL_STEPS_PER_MIN)).apply {
            addRow(arrayOf<Any>(if (pace.playing) 1 else 0, pace.stepsPerMin))
        }
    }
    ...
    companion object {
        ...
        val CURRENT_URI: Uri = Uri.parse("content://$AUTHORITY/current")
